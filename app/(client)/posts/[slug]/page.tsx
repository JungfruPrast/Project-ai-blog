      ...
      const renderText = (text: string, marks?: string[]): JSX.Element | string => {
        ...
        return marks.reduce((acc: JSX.Element | string, mark: string) => {
          ...
          switch (mark) {
            ...
            case 'code':
              return <code key={_key} className='custom-code-style dark:bg-zinc-800 dark:rounded-md dark:text-white'>{acc}</code>;
            default:
              return acc;
          }
        }, text);
    ...
    codeBlock: ({ value }: { value: CodeBlockValue }) => {
      ...
      return (
        <div className="relative">
          <pre className="text-inherit md:flex overflow-auto p-3 my-2 rounded-lg w-auto h-2/6 bg-prismjs-default shadow-md dark:shadow-gray-700 mt-2 z-10">
